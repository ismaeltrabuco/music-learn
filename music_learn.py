...
def detect_guitar(frame):
    if YOLO:
        try:
            ...
            for r in results:
                for box in r.boxes:
                    if r.names.get(int(box.cls), '') == 'guitar':  # Adjust class ID if needed
...
def get_frame_with_annotations(frame, faces, guitar_box, posture_ok):
    ...
    if guitar_box:
        ...
        if abs(angle) > 30:
            cv2.putText(frame, "Adjust guitar angle!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    if not posture_ok:
        cv2.putText(frame, "Center posture!", (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    return frame
...
if start_button:
    ...
    if not cap.isOpened():
        st.error('Error: Camera not available. Please test locally or check device.')
        st.stop()
    ...
    if frames:
        ...
        if pinata_jwt:
            ...
            try:
                hash = upload_to_pinata(json_buffer, f'report_{int(time.time())}.json', 'application/json', pinata_jwt)
